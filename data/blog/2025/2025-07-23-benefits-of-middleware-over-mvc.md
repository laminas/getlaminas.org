...
The traditional **Model-View-Controller (MVC)** pattern is a tried and tested paradigm for developing web applications.
However, recent industry shifts, including growing adoption of async architectures, prove challenging to accommodate in an MVC application.
...
Middleware components can be simpler and more focused on a given task, which promotes readability and maintainability even in complex platforms.

The tangled controller classes ('God controllers') found in MVC often perform too many tasks, like authorizing users, handling forms, formatting the response, and picking views.
This can result in duplication across routes.
Middleware instead handles **one thing only** like authentication, CSRF handling, or requests logging.
Given the reduced complexity per unit, the logic becomes simpler, there are fewer side effects and less code to rummage around in when debugging and maintaining code.
...
For example, if a certain middleware can fully handle a request, it can return the response early, reducing execution time and computational requirements.
...
This comes from the fact that it's built as a pipeline of isolated, ordered units which makes it easy to rearrange logic without affecting unrelated code.
...
You can define a custom workflow for any given route, based on that route's needs.
...
- You start with the file upload handler.
- Add authentication.
- Add file parsing middleware.

This feature satisfies the different needs of each route, while avoiding unnecessary logic and defining each route’s behavior in a clear and predictable way.
...
The bottom line is that middleware:

- Doesn't need view rendering.
- Is JSON-first.
...
It's called the **strangle strategy** (or [Strangler Fig](https://martinfowler.com/bliki/StranglerFigApplication.html) pattern).
